...
export class VentaService {
  ...
  async emitirDte(payload: any) {
    ...
    const empresaDemo = {
      ...
      logo: path.join(process.cwd(), 'src', 'venta', 'CocaCola.png'), // ✅ logo local
    };
    ...
    // Solo preview indica logo
    buffers.push(textBuf(`[LOGO: ${empresaDemo.logo}]\n`));

    buffers.push(
      textBuf(`${empresaDemo.direccion}, ${empresaDemo.comuna}\n`),
    );
    ...
    buffers.push(
      textBuf(
        `Neto:`.padEnd(ANCHO_NOMBRE) +
          formatCLP(neto).padStart(ANCHO_PRECIO) +
          '\n',
      ),
    );
    buffers.push(
      textBuf(
        `IVA (19%):`.padEnd(ANCHO_NOMBRE) +
          formatCLP(iva).padStart(ANCHO_PRECIO) +
          '\n',
      ),
    );
    buffers.push(BOLD_ON);
    ...
    buffers.push(
      textBuf(
        `TOTAL:`.padEnd(ANCHO_NOMBRE) +
          formatCLP(total).padStart(ANCHO_PRECIO) +
          '\n',
      ),
    );
    buffers.push(RESET_HW);
    ...
    return {
      ...
      pdf417Base64, // para mostrar en frontend
